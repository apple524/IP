...
const withAuthorizationToken = (apiUrl, token) => {
...
const redactAuthorizationToken = (apiUrl) => {
	let urlObj;
	try {
		urlObj = new URL(apiUrl);
	} catch {
	...
	if (!urlObj.searchParams.has("jwt")) return apiUrl;
	urlObj.searchParams.set("jwt", REDACTED);
	return urlObj.href;
};
const AUTHORIZABLE_URLS = [
	"downloadApiUrl",
	"uploadApiUrl",
...
const applyAuthorizationToken = (config) => {
	...
	#nextMeasurement() {
		...
		fetch(url, {
			...
			if (this.#retries++ < MAX_RETRIES) this.#nextMeasurement();
				...
				this.#retries = 0;
				this.#setRunning(false);
				this.#onConnectionError(`Connection failed to ${safeUrl}. Gave up after ${MAX_RETRIES} retries.`);
			}
		});
	}
...
var MeasurementEngine = class {
	constructor(userConfig = {}) {
		this.#config = applyAuthorizationToken(Object.assign({}, defaultConfig, userConfig, internalConfig));
